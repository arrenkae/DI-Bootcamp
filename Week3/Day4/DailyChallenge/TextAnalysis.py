...
class Text:
    ...
    def wordcount(self):
        ...
        'ain', 'aren', 'couldn', 'didn', 'doesn', 'hadn', 'hasn', 'haven', 'isn', 'ma', 'mightn', 'mustn', 'needn', 'shan', 'shouldn', 'wasn', 'weren', 'won' 'wouldn', 'would']
        remove = '''!()-—[]{};:'"\,<>./?@#$%^&*_~♦1234567890'''
        words = []
        for word in self.text.lower().split():
            word_clean = word.lower().strip(remove).replace("'s",'').replace("'d",'').replace("'t",'').replace("'ll",'').replace("'re",'').replace("'ve",'')
